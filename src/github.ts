...
import { githubRegexLink, safeSlice } from "./extra";

export const githubComposer = new Composer();

// Thanks to https://github.com/xhyrom/bun-discord-bot
githubComposer.hears(githubRegexLink, async ctx => {
	...
	return ctx.reply(
		[
			`<code>${path?.split("/").slice(1).join("/").replace(/%2F/g, "/")}</code> — <b>(L${firstLineNumber + 1}${
